...
namespace BattleShip {
    ...
    void HumanPlayer::place_ships(Board &board, const int &rows, const int &cols) {
        board.print();
        ...
        for (const auto &ship : ShipsMap) {
            do {
                ...
                gotValidInt = getValidInts(
                     ...
                     !board.check_available_space(rowInput, colInput, shipSize, orientation));
    ...
    void HumanPlayer::getMove(int &rowInput, int &colInput, int rows, int cols) {
        ...
        do {  // Ask the player for their move
            ...
            if (gotValidInt && !firing_board.validAction(rowInput, colInput, rows,
                                                         cols)) { // If they enter in 2 numbers, check if they are in bounds
                std::cout << this->getName() << "'s Firing Board" << std::endl;
                firing_board.print();
                std::cout << std::endl << std::endl << this->getName() << "'s Placement Board" << std::endl;

...
namespace BattleShip {
    ...
    void SearchAndDestroyAI::getMove(int &rowInput, int &colInput, int rows, int cols) {
        if (surrounding.empty()) { // Checks to see if the vector for Destroy Mode is empty
            RandomAI::getMove(rowInput, colInput, rows, cols);
            if (opponent->getPlacementBoard().getChar(rowInput, colInput) != '*') {  //  if its a hit, get all the surrounding locations
                getSurroundingSpace(rowInput, colInput, rows, cols); // Fills up the Destroy mode's vector
            }
        } else {
            destroyMode(rowInput, colInput, rows, cols);  //  returns the next surrounding spot
        }
    }

    //  gets the left, top, right, and bottom spaces around a location
    void SearchAndDestroyAI::getSurroundingSpace(const int &rowInput, const int &colInput, const int &rows, const int &cols) {

        if (colInput - 1 >= 0 && opponent->getPlacementBoard().validAction(rowInput, colInput - 1, rows, cols)) {  // checks bounds
        ...
        if (rowInput - 1 >= 0 && opponent->getPlacementBoard().validAction(rowInput - 1, colInput, rows, cols)) {
            if(!isIn(rowInput - 1, colInput)) {
                surrounding.push_back({rowInput - 1, colInput});
                removeFromRandom(rowInput - 1, colInput);
            }
        }
        if (colInput + 1 < cols && opponent->getPlacementBoard().validAction(rowInput, colInput + 1, rows, cols)) {
            if(!isIn(rowInput, colInput + 1)) {
                surrounding.push_back({rowInput, colInput + 1});
                removeFromRandom(rowInput, colInput + 1);
            }
        }
        if (rowInput + 1 < rows && opponent->getPlacementBoard().validAction(rowInput + 1, colInput, rows, cols)) {
            if(!isIn(rowInput + 1, colInput)) {
                surrounding.push_back({rowInput + 1, colInput});
                removeFromRandom(rowInput + 1, colInput);
            }
    ...
    void SearchAndDestroyAI::destroyMode(int &rowInput, int &colInput, int rows, int cols) {
        rowInput = surrounding.at(0).first;
        colInput = surrounding.at(0).second;
        if (opponent->getPlacementBoard().getChar(rowInput, colInput) != '*') {
            getSurroundingSpace(rowInput, colInput, rows, cols);
        }

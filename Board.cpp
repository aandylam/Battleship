...
// Function to check whether the given row and column input already has a ship on it
    bool BattleShip::Board::check_available_space(const int &rowInput, const int &columnInput, const int &shipSize,
                                                  const char &orientation) {
        if (orientation == 'H' || orientation == 'h') {
...
//  checks to see if the firing input was in bounds, and to see if the user has not already fired there
    bool BattleShip::Board::validAction(const int &row, const int &col, const int &rowSize, const int &colSize) {
...
// Function that checks what ship is on the specified row and column input
    char BattleShip::Board::obtainShipSpace(char &shipName, const int &row, const int &col) {
